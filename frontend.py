...
class Gui():
    ...
    btnUptade = Button(window, text="Atualizar")
    btnDell = Button(window, text='Excluir')
    btnClose = Button(window, text="Fechar")

    lblNome.grid(row=0, column=0)
    lblSobrenome.grid(row=1)
    lblEmail.grid(row=2)
    lblCPF.grid(row=3)
    ...
    listClientes.grid(row=0, column=6, rowspan=10)
    ...
    btnUptade.grid(row=7, column=0, columnspan=2)
